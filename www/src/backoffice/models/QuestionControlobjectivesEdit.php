 ...
 * Page class
 */
class QuestionControlobjectivesEdit extends QuestionControlobjectives
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "QuestionControlobjectivesEdit";
    ...
    public function run()
    {
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("controlObj_name") ?? Key(0) ?? Route(2)) !== null) {
                $this->controlObj_name->setQueryStringValue($keyValue);
                $this->controlObj_name->setOldValue($this->controlObj_name->QueryStringValue);
            } elseif (Post("controlObj_name") !== null) {
                $this->controlObj_name->setFormValue(Post("controlObj_name"));
                $this->controlObj_name->setOldValue($this->controlObj_name->FormValue);
            } else {
            ...
            if ($this->isShow()) {
                ...
                if ($this->TotalRecords <= 0) { // No record found
                    ...
                    if ($this->controlObj_name->CurrentValue != null) {
                        while (!$rs->EOF) {
                            if (SameString($this->controlObj_name->CurrentValue, $rs->fields['controlObj_name'])) {
