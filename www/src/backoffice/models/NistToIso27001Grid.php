 ...
 * Page class
 */
class NistToIso27001Grid extends NistToIso27001
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "NistToIso27001Grid";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fnist_to_iso27001grid";
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_nistrefs_family") && $CurrentForm->hasValue("o_nistrefs_family") && $this->nistrefs_family->CurrentValue != $this->nistrefs_family->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_isorefs") && $CurrentForm->hasValue("o_isorefs") && $this->isorefs->CurrentValue != $this->isorefs->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_just_for_question_link") && $CurrentForm->hasValue("o_just_for_question_link") && $this->just_for_question_link->CurrentValue != $this->just_for_question_link->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->nistrefs_family->clearErrorMessage();
        $this->isorefs->clearErrorMessage();
        $this->just_for_question_link->clearErrorMessage();
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_nistrefs_family")) {
            $this->nistrefs_family->setOldValue($CurrentForm->getValue("o_nistrefs_family"));
        }
        ...
        if ($CurrentForm->hasValue("o_isorefs")) {
            $this->isorefs->setOldValue($CurrentForm->getValue("o_isorefs"));
        }
        ...
        if ($CurrentForm->hasValue("o_just_for_question_link")) {
            $this->just_for_question_link->setOldValue($CurrentForm->getValue("o_just_for_question_link"));
        }
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->getCurrentMasterTable() == "nist_refs_controlfamily") {
            $this->nistrefs_family->CurrentValue = $this->nistrefs_family->getSessionValue();
        }
        if ($this->getCurrentMasterTable() == "iso27001_refs") {
            $this->isorefs->CurrentValue = $this->isorefs->getSessionValue();
        }
        $conn = $this->getConnection();
    ...
    protected function setupMasterParms()
    {
        ...
        if ($masterTblVar == "nist_refs_controlfamily") {
            $masterTbl = Container("nist_refs_controlfamily");
            $this->nistrefs_family->Visible = false;
