...
loadjs.ready("head", function () {
    ...
    fnist_refsedit = currentForm = new ew.Form("fnist_refsedit", "edit");
    ...
    // Validate form
    fnist_refsedit.validate = function () {
    ...
    // Form_CustomValidate
    fnist_refsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fnist_refsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fnist_refsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fnist_refsedit" id="fnist_refsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="nist_refs" data-field="x_Nidentifier" data-hidden="1" name="o_Nidentifier" id="o_Nidentifier" value="<?= HtmlEncode($Page->Nidentifier->OldValue ?? $Page->Nidentifier->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fnist_refsedit", "editor"], function() {
	ew.createEditor("fnist_refsedit", "x_control_description", 35, 4, <?= $Page->control_description->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
...
<script>
loadjs.ready(["fnist_refsedit", "editor"], function() {
	ew.createEditor("fnist_refsedit", "x_discussion", 35, 4, <?= $Page->discussion->ReadOnly || false ? "true" : "false" ?>);
});
</script>
</span>
</div></div>
