...
loadjs.ready("head", function () {
    ...
    fthreatsedit = currentForm = new ew.Form("fthreatsedit", "edit");
    ...
    // Validate form
    fthreatsedit.validate = function () {
    ...
    // Form_CustomValidate
    fthreatsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fthreatsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fthreatsedit.lists.Confidentiality = <?= $Page->Confidentiality->toClientList($Page) ?>;
    fthreatsedit.lists.Integrity = <?= $Page->Integrity->toClientList($Page) ?>;
    fthreatsedit.lists.Availability = <?= $Page->Availability->toClientList($Page) ?>;
    fthreatsedit.lists.Efficiency = <?= $Page->Efficiency->toClientList($Page) ?>;
    loadjs.done("fthreatsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fthreatsedit" id="fthreatsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="threats" data-field="x_name" data-hidden="1" name="o_name" id="o_name" value="<?= HtmlEncode($Page->name->OldValue ?? $Page->name->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fthreatsedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fthreatsedit", "x_created_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fthreatsedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fthreatsedit", "x_updated_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
