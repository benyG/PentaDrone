...
// Page object
$QuestionControlobjectivesEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fquestion_controlobjectivesedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fquestion_controlobjectivesedit = currentForm = new ew.Form("fquestion_controlobjectivesedit", "edit");
    ...
    fquestion_controlobjectivesedit.addFields([
    ...
    // Set invalid fields
    $(function() {
        var f = fquestion_controlobjectivesedit,
            fobj = f.getForm(),
            $fobj = $(fobj),
    ...
    // Validate form
    fquestion_controlobjectivesedit.validate = function () {
    ...
    // Form_CustomValidate
    fquestion_controlobjectivesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fquestion_controlobjectivesedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fquestion_controlobjectivesedit.lists.question_domain_id = <?= $Page->question_domain_id->toClientList($Page) ?>;
    fquestion_controlobjectivesedit.lists.layer_id = <?= $Page->layer_id->toClientList($Page) ?>;
    fquestion_controlobjectivesedit.lists.function_csf = <?= $Page->function_csf->toClientList($Page) ?>;
    loadjs.done("fquestion_controlobjectivesedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fquestion_controlobjectivesedit" id="fquestion_controlobjectivesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="question_controlobjectives" data-field="x_controlObj_name" data-hidden="1" name="o_controlObj_name" id="o_controlObj_name" value="<?= HtmlEncode($Page->controlObj_name->OldValue ?? $Page->controlObj_name->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fquestion_controlobjectivesedit"], function() {
    fquestion_controlobjectivesedit.createAutoSuggest(Object.assign({"id":"x_function_csf","forceSelect":false}, ew.vars.tables.question_controlobjectives.fields.function_csf.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["fquestion_controlobjectivesedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fquestion_controlobjectivesedit", "x_created_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fquestion_controlobjectivesedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fquestion_controlobjectivesedit", "x_updated_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
