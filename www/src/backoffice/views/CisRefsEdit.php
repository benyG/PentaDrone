...
loadjs.ready("head", function () {
    ...
    fcis_refsedit = currentForm = new ew.Form("fcis_refsedit", "edit");
    ...
    // Validate form
    fcis_refsedit.validate = function () {
    ...
    // Form_CustomValidate
    fcis_refsedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcis_refsedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fcis_refsedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fcis_refsedit" id="fcis_refsedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="cis_refs" data-field="x_Nidentifier" data-hidden="1" name="o_Nidentifier" id="o_Nidentifier" value="<?= HtmlEncode($Page->Nidentifier->OldValue ?? $Page->Nidentifier->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
