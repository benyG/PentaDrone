...
// Page object
$RiskLibrairiesEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var frisk_librairiesedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    frisk_librairiesedit = currentForm = new ew.Form("frisk_librairiesedit", "edit");
    ...
    frisk_librairiesedit.addFields([
    ...
    // Validate form
    frisk_librairiesedit.validate = function () {
    ...
    // Form_CustomValidate
    frisk_librairiesedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    frisk_librairiesedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    frisk_librairiesedit.lists.layer = <?= $Page->layer->toClientList($Page) ?>;
    frisk_librairiesedit.lists.function_csf = <?= $Page->function_csf->toClientList($Page) ?>;
    frisk_librairiesedit.lists.Confidentiality = <?= $Page->Confidentiality->toClientList($Page) ?>;
    frisk_librairiesedit.lists.Integrity = <?= $Page->Integrity->toClientList($Page) ?>;
    frisk_librairiesedit.lists.Availability = <?= $Page->Availability->toClientList($Page) ?>;
    frisk_librairiesedit.lists.Efficiency = <?= $Page->Efficiency->toClientList($Page) ?>;
    loadjs.done("frisk_librairiesedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="frisk_librairiesedit" id="frisk_librairiesedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_risk_librairies_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_risk_librairies_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="risk_librairies" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["frisk_librairiesedit"], function() {
    frisk_librairiesedit.createAutoSuggest(Object.assign({"id":"x_layer","forceSelect":false}, ew.vars.tables.risk_librairies.fields.layer.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["frisk_librairiesedit"], function() {
    frisk_librairiesedit.createAutoSuggest(Object.assign({"id":"x_function_csf","forceSelect":false}, ew.vars.tables.risk_librairies.fields.function_csf.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready(["frisk_librairiesedit", "datetimepicker"], function() {
    ew.createDateTimePicker("frisk_librairiesedit", "x_created_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["frisk_librairiesedit", "datetimepicker"], function() {
    ew.createDateTimePicker("frisk_librairiesedit", "x_updated_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
