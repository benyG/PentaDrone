...
// Page object
$SubcatNistLinksEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fsubcat_nist_linksedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fsubcat_nist_linksedit = currentForm = new ew.Form("fsubcat_nist_linksedit", "edit");
    ...
    fsubcat_nist_linksedit.addFields([
    ...
    // Validate form
    fsubcat_nist_linksedit.validate = function () {
    ...
    // Form_CustomValidate
    fsubcat_nist_linksedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsubcat_nist_linksedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fsubcat_nist_linksedit.lists.subcat_id = <?= $Page->subcat_id->toClientList($Page) ?>;
    fsubcat_nist_linksedit.lists.nistrefs_id = <?= $Page->nistrefs_id->toClientList($Page) ?>;
    loadjs.done("fsubcat_nist_linksedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fsubcat_nist_linksedit" id="fsubcat_nist_linksedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="subcat_nist_links" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fsubcat_nist_linksedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fsubcat_nist_linksedit", "x_created_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fsubcat_nist_linksedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fsubcat_nist_linksedit", "x_updated_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
