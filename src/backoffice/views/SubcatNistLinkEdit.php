...
// Page object
$SubcatNistLinkEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fsubcat_nist_linkedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fsubcat_nist_linkedit = currentForm = new ew.Form("fsubcat_nist_linkedit", "edit");
    ...
    fsubcat_nist_linkedit.addFields([
    ...
    // Validate form
    fsubcat_nist_linkedit.validate = function () {
    ...
    // Form_CustomValidate
    fsubcat_nist_linkedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsubcat_nist_linkedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fsubcat_nist_linkedit.lists.nistrefs_id = <?= $Page->nistrefs_id->toClientList($Page) ?>;
    loadjs.done("fsubcat_nist_linkedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fsubcat_nist_linkedit" id="fsubcat_nist_linkedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<?php if ($Page->id->Visible) { // id ?>
    <div id="r_id" class="form-group row">
        <label id="elh_subcat_nist_link_id" class="<?= $Page->LeftColumnClass ?>"><?= $Page->id->caption() ?><?= $Page->id->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->id->cellAttributes() ?>>
<span id="el_subcat_nist_link_id">
<span<?= $Page->id->viewAttributes() ?>>
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->id->getDisplayValue($Page->id->EditValue))) ?>"></span>
</span>
<input type="hidden" data-table="subcat_nist_link" data-field="x_id" data-hidden="1" name="x_id" id="x_id" value="<?= HtmlEncode($Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
