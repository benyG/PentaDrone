...
// Page object
$SubcatCobitLinksEdit = &$Page;
?>
<script>
var currentForm, currentPageID;
var fsubcat_cobit_linksedit;
loadjs.ready("head", function () {
    var $ = jQuery;
    // Form object
    currentPageID = ew.PAGE_ID = "edit";
    fsubcat_cobit_linksedit = currentForm = new ew.Form("fsubcat_cobit_linksedit", "edit");
    ...
    fsubcat_cobit_linksedit.addFields([
    ...
    // Validate form
    fsubcat_cobit_linksedit.validate = function () {
    ...
    // Form_CustomValidate
    fsubcat_cobit_linksedit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fsubcat_cobit_linksedit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    fsubcat_cobit_linksedit.lists.subcat_id = <?= $Page->subcat_id->toClientList($Page) ?>;
    fsubcat_cobit_linksedit.lists.cobitrefs_id = <?= $Page->cobitrefs_id->toClientList($Page) ?>;
    loadjs.done("fsubcat_cobit_linksedit");
});
</script>
<script>
loadjs.ready("head", function () {
...
<?php } ?>
<form name="fsubcat_cobit_linksedit" id="fsubcat_cobit_linksedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="subcat_cobit_links" data-field="x_id" data-hidden="1" name="o_id" id="o_id" value="<?= HtmlEncode($Page->id->OldValue ?? $Page->id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fsubcat_cobit_linksedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fsubcat_cobit_linksedit", "x_created_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fsubcat_cobit_linksedit", "datetimepicker"], function() {
    ew.createDateTimePicker("fsubcat_cobit_linksedit", "x_updated_at", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
