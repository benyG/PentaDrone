 ...
 * Page class
 */
class QuestionsLibraryList extends QuestionsLibrary
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "QuestionsLibraryList";

    // Rendering View
    public $RenderingView = false;

    // Grid form hidden field names
    public $FormName = "fquestions_librarylist";
    ...
    public function __construct()
    {
        ...
        $this->AddUrl = "QuestionsLibraryAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiDeleteUrl = "QuestionsLibraryDelete";
        $this->MultiUpdateUrl = "QuestionsLibraryUpdate";
        ...
        $this->FilterOptions->TagClassName = "ew-filter-option fquestions_librarylistsrch";
    ...
    public function run()
    {
        ...
        $this->Evidence_to_request->Visible = false;
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "iso27001_refs") {
            $masterTbl = Container("iso27001_refs");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("Iso27001RefsList"); // Return to master page
                return;
            } else {
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "nist_to_iso27001") {
            $masterTbl = Container("nist_to_iso27001");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("NistToIso27001List"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = ROWTYPE_MASTER; // Master row
        ...
        // Load master record
        if ($this->CurrentMode != "add" && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "question_controlobjectives") {
            $masterTbl = Container("question_controlobjectives");
            $rsmaster = $masterTbl->loadRs($this->DbMasterFilter)->fetch(\PDO::FETCH_ASSOC);
            $this->MasterRecordExists = $rsmaster !== false;
            if (!$this->MasterRecordExists) {
                $this->setFailureMessage($Language->phrase("NoRecord")); // Set no record found
                $this->terminate("QuestionControlobjectivesList"); // Return to master page
                return;
            } else {
                $masterTbl->loadListRowValues($rsmaster);
                $masterTbl->RowType = ROWTYPE_MASTER; // Master row
    ...
    {
        global $CurrentForm;
        if ($CurrentForm->hasValue("x_libelle") && $CurrentForm->hasValue("o_libelle") && $this->libelle->CurrentValue != $this->libelle->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_controlObj_id") && $CurrentForm->hasValue("o_controlObj_id") && $this->controlObj_id->CurrentValue != $this->controlObj_id->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_refs1") && $CurrentForm->hasValue("o_refs1") && $this->refs1->CurrentValue != $this->refs1->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_refs2") && $CurrentForm->hasValue("o_refs2") && $this->refs2->CurrentValue != $this->refs2->OldValue) {
            return false;
        }
        if ($CurrentForm->hasValue("x_Activation_status") && $CurrentForm->hasValue("o_Activation_status") && $this->Activation_status->CurrentValue != $this->Activation_status->OldValue) {
            return false;
        }
        return true;
    }
    ...
    public function resetFormError()
    {
        ...
        $this->libelle->clearErrorMessage();
        $this->controlObj_id->clearErrorMessage();
        $this->refs1->clearErrorMessage();
        $this->refs2->clearErrorMessage();
        $this->Activation_status->clearErrorMessage();
    }
    ...
    public function getFilterList()
    {
        ...
        $filterList = Concat($filterList, $this->libelle->AdvancedSearch->toJson(), ","); // Field libelle
        $filterList = Concat($filterList, $this->Evidence_to_request->AdvancedSearch->toJson(), ","); // Field Evidence_to_request
        $filterList = Concat($filterList, $this->controlObj_id->AdvancedSearch->toJson(), ","); // Field controlObj_id
        $filterList = Concat($filterList, $this->created_at->AdvancedSearch->toJson(), ","); // Field created_at
        $filterList = Concat($filterList, $this->updated_at->AdvancedSearch->toJson(), ","); // Field updated_at
        $filterList = Concat($filterList, $this->refs1->AdvancedSearch->toJson(), ","); // Field refs1
        $filterList = Concat($filterList, $this->refs2->AdvancedSearch->toJson(), ","); // Field refs2
        $filterList = Concat($filterList, $this->Activation_status->AdvancedSearch->toJson(), ","); // Field Activation_status
        if ($this->BasicSearch->Keyword != "") {
    ...
    protected function processFilterList()
    {
        ...
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            $UserProfile->setSearchFilters(CurrentUserName(), "fquestions_librarylistsrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field libelle
        $this->libelle->AdvancedSearch->SearchValue = @$filter["x_libelle"];
        $this->libelle->AdvancedSearch->SearchOperator = @$filter["z_libelle"];
        $this->libelle->AdvancedSearch->SearchCondition = @$filter["v_libelle"];
        $this->libelle->AdvancedSearch->SearchValue2 = @$filter["y_libelle"];
        $this->libelle->AdvancedSearch->SearchOperator2 = @$filter["w_libelle"];
        $this->libelle->AdvancedSearch->save();

        // Field Evidence_to_request
        $this->Evidence_to_request->AdvancedSearch->SearchValue = @$filter["x_Evidence_to_request"];
        $this->Evidence_to_request->AdvancedSearch->SearchOperator = @$filter["z_Evidence_to_request"];
        $this->Evidence_to_request->AdvancedSearch->SearchCondition = @$filter["v_Evidence_to_request"];
        $this->Evidence_to_request->AdvancedSearch->SearchValue2 = @$filter["y_Evidence_to_request"];
        $this->Evidence_to_request->AdvancedSearch->SearchOperator2 = @$filter["w_Evidence_to_request"];
        $this->Evidence_to_request->AdvancedSearch->save();

        // Field controlObj_id
        $this->controlObj_id->AdvancedSearch->SearchValue = @$filter["x_controlObj_id"];
        $this->controlObj_id->AdvancedSearch->SearchOperator = @$filter["z_controlObj_id"];
        $this->controlObj_id->AdvancedSearch->SearchCondition = @$filter["v_controlObj_id"];
        $this->controlObj_id->AdvancedSearch->SearchValue2 = @$filter["y_controlObj_id"];
        $this->controlObj_id->AdvancedSearch->SearchOperator2 = @$filter["w_controlObj_id"];
        $this->controlObj_id->AdvancedSearch->save();
        ...
        // Field refs1
        $this->refs1->AdvancedSearch->SearchValue = @$filter["x_refs1"];
        $this->refs1->AdvancedSearch->SearchOperator = @$filter["z_refs1"];
        $this->refs1->AdvancedSearch->SearchCondition = @$filter["v_refs1"];
        $this->refs1->AdvancedSearch->SearchValue2 = @$filter["y_refs1"];
        $this->refs1->AdvancedSearch->SearchOperator2 = @$filter["w_refs1"];
        $this->refs1->AdvancedSearch->save();

        // Field refs2
        $this->refs2->AdvancedSearch->SearchValue = @$filter["x_refs2"];
        $this->refs2->AdvancedSearch->SearchOperator = @$filter["z_refs2"];
        $this->refs2->AdvancedSearch->SearchCondition = @$filter["v_refs2"];
        $this->refs2->AdvancedSearch->SearchValue2 = @$filter["y_refs2"];
        $this->refs2->AdvancedSearch->SearchOperator2 = @$filter["w_refs2"];
        $this->refs2->AdvancedSearch->save();

        // Field Activation_status
        $this->Activation_status->AdvancedSearch->SearchValue = @$filter["x_Activation_status"];
        $this->Activation_status->AdvancedSearch->SearchOperator = @$filter["z_Activation_status"];
        $this->Activation_status->AdvancedSearch->SearchCondition = @$filter["v_Activation_status"];
        $this->Activation_status->AdvancedSearch->SearchValue2 = @$filter["y_Activation_status"];
        $this->Activation_status->AdvancedSearch->SearchOperator2 = @$filter["w_Activation_status"];
        $this->Activation_status->AdvancedSearch->save();
    ...
    {
        $where = "";
        $this->buildBasicSearchSql($where, $this->libelle, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->Evidence_to_request, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->controlObj_id, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->refs1, $arKeywords, $type);
        $this->buildBasicSearchSql($where, $this->refs2, $arKeywords, $type);
        return $where;
    }
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->libelle); // libelle
            $this->updateSort($this->controlObj_id); // controlObj_id
            $this->updateSort($this->refs1); // refs1
            $this->updateSort($this->refs2); // refs2
            $this->updateSort($this->Activation_status); // Activation_status
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->id->setSort("");
                $this->libelle->setSort("");
                $this->Evidence_to_request->setSort("");
                $this->controlObj_id->setSort("");
                $this->created_at->setSort("");
                $this->updated_at->setSort("");
                $this->refs1->setSort("");
                $this->refs2->setSort("");
                $this->Activation_status->setSort("");
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fquestions_librarylistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fquestions_librarylistsrch\" href=\"#\" onclick=\"return false;\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
        ...
        if (!$this->isGridAdd() && !$this->isGridEdit()) { // Not grid add/edit mode
            ...
            foreach ($this->ListActions->Items as $listaction) {
                if ($listaction->Select == ACTION_MULTIPLE) {
                    ...
                    $item->Body = '<a class="ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" href="#" onclick="return ew.submitAction(event,jQuery.extend({f:document.fquestions_librarylist},' . $listaction->toJson(true) . '));">' . $icon . '</a>';
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->libelle->CurrentValue = null;
        $this->libelle->OldValue = $this->libelle->CurrentValue;
        $this->Evidence_to_request->CurrentValue = null;
        $this->Evidence_to_request->OldValue = $this->Evidence_to_request->CurrentValue;
        $this->controlObj_id->CurrentValue = null;
        $this->controlObj_id->OldValue = $this->controlObj_id->CurrentValue;
        $this->created_at->CurrentValue = null;
        $this->created_at->OldValue = $this->created_at->CurrentValue;
        $this->updated_at->CurrentValue = null;
        $this->updated_at->OldValue = $this->updated_at->CurrentValue;
        $this->refs1->CurrentValue = null;
        $this->refs1->OldValue = $this->refs1->CurrentValue;
        $this->refs2->CurrentValue = null;
        $this->refs2->OldValue = $this->refs2->CurrentValue;
        $this->Activation_status->CurrentValue = null;
        $this->Activation_status->OldValue = $this->Activation_status->CurrentValue;
    }
    ...
    protected function loadFormValues()
    {
        ...
        if ($CurrentForm->hasValue("o_libelle")) {
            $this->libelle->setOldValue($CurrentForm->getValue("o_libelle"));
        }
        ...
        if ($CurrentForm->hasValue("o_controlObj_id")) {
            $this->controlObj_id->setOldValue($CurrentForm->getValue("o_controlObj_id"));
        }
        ...
        if ($CurrentForm->hasValue("o_refs1")) {
            $this->refs1->setOldValue($CurrentForm->getValue("o_refs1"));
        }
        ...
        if ($CurrentForm->hasValue("o_refs2")) {
            $this->refs2->setOldValue($CurrentForm->getValue("o_refs2"));
        }
        ...
        if ($CurrentForm->hasValue("o_Activation_status")) {
            $this->Activation_status->setOldValue($CurrentForm->getValue("o_Activation_status"));
        }
    }
    ...
    protected function newRow()
    {
        ...
        $row['libelle'] = $this->libelle->CurrentValue;
        $row['Evidence_to_request'] = $this->Evidence_to_request->CurrentValue;
        $row['controlObj_id'] = $this->controlObj_id->CurrentValue;
        $row['created_at'] = $this->created_at->CurrentValue;
        $row['updated_at'] = $this->updated_at->CurrentValue;
        $row['refs1'] = $this->refs1->CurrentValue;
        $row['refs2'] = $this->refs2->CurrentValue;
        $row['Activation_status'] = $this->Activation_status->CurrentValue;
        return $row;
    }
    ...
    protected function addRow($rsold = null)
    {
        ...
        // Check referential integrity for master table 'questions_library'
        $validMasterRecord = true;
        $masterFilter = $this->sqlMasterFilter_question_controlobjectives();
        if (strval($this->controlObj_id->CurrentValue) != "") {
            $masterFilter = str_replace("@controlObj_name@", AdjustSql($this->controlObj_id->CurrentValue, "DB"), $masterFilter);
        } else {
        ...
        // libelle
        $this->libelle->setDbValueDef($rsnew, $this->libelle->CurrentValue, "", false);

        // controlObj_id
        $this->controlObj_id->setDbValueDef($rsnew, $this->controlObj_id->CurrentValue, "", false);

        // refs1
        $this->refs1->setDbValueDef($rsnew, $this->refs1->CurrentValue, "", false);

        // refs2
        $this->refs2->setDbValueDef($rsnew, $this->refs2->CurrentValue, "", false);

        // Activation_status
        $this->Activation_status->setDbValueDef($rsnew, $this->Activation_status->CurrentValue, 0, false);
    ...
    protected function getExportTag($type, $custom = false)
    {
        ...
        if (SameText($type, "excel")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" onclick=\"return ew.export(document.fquestions_librarylist, '" . $this->ExportExcelUrl . "', 'excel', true);\">" . $Language->phrase("ExportToExcel") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportExcelUrl . "\" class=\"ew-export-link ew-excel\" title=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToExcelText")) . "\">" . $Language->phrase("ExportToExcel") . "</a>";
            }
        } elseif (SameText($type, "word")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" onclick=\"return ew.export(document.fquestions_librarylist, '" . $this->ExportWordUrl . "', 'word', true);\">" . $Language->phrase("ExportToWord") . "</a>";
            } else {
                return "<a href=\"" . $this->ExportWordUrl . "\" class=\"ew-export-link ew-word\" title=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToWordText")) . "\">" . $Language->phrase("ExportToWord") . "</a>";
            }
        } elseif (SameText($type, "pdf")) {
            if ($custom) {
                return "<a href=\"#\" class=\"ew-export-link ew-pdf\" title=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" data-caption=\"" . HtmlEncode($Language->phrase("ExportToPDFText")) . "\" onclick=\"return ew.export(document.fquestions_librarylist, '" . $this->ExportPdfUrl . "', 'pdf', true);\">" . $Language->phrase("ExportToPDF") . "</a>";
            } else {
            ...
            return '<button id="emf_questions_library" class="ew-export-link ew-email" title="' . $Language->phrase("ExportToEmailText") . '" data-caption="' . $Language->phrase("ExportToEmailText") . '" onclick="ew.emailDialogShow({lnk:\'emf_questions_library\', hdr:ew.language.phrase(\'ExportToEmailText\'), f:document.fquestions_librarylist, sel:false' . $url . '});">' . $Language->phrase("ExportToEmail") . '</button>';
    ...
    protected function setupSearchSortOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" href=\"#\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fquestions_librarylistsrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
    ...
    */
    public function exportData($return = false)
    {
        ...
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "iso27001_refs") {
            $iso27001_refs = Container("iso27001_refs");
            $rsmaster = $iso27001_refs->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $iso27001_refs;
                    $iso27001_refs->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
        ...
        // Export master record
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "nist_to_iso27001") {
            $nist_to_iso27001 = Container("nist_to_iso27001");
            $rsmaster = $nist_to_iso27001->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $nist_to_iso27001;
                    $nist_to_iso27001->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
                $doc->setStyle($exportStyle); // Restore
        ...
        // Export master record
        if (Config("EXPORT_MASTER_RECORD") && $this->getMasterFilter() != "" && $this->getCurrentMasterTable() == "question_controlobjectives") {
            $question_controlobjectives = Container("question_controlobjectives");
            $rsmaster = $question_controlobjectives->loadRs($this->DbMasterFilter); // Load master record
            if ($rsmaster) {
                $exportStyle = $doc->Style;
                $doc->setStyle("v"); // Change to vertical
                if (!$this->isExport("csv") || Config("EXPORT_MASTER_RECORD_FOR_CSV")) {
                    $doc->Table = $question_controlobjectives;
                    $question_controlobjectives->exportDocument($doc, new Recordset($rsmaster));
                    $doc->exportEmptyRow();
                    $doc->Table = &$this;
                }
                $doc->setStyle($exportStyle); // Restore
