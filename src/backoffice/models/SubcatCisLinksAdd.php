 ...
 * Page class
 */
class SubcatCisLinksAdd extends SubcatCisLinks
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SubcatCisLinksAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("SubcatCisLinksList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "SubcatCisLinksList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "SubcatCisLinksView") {
    ...
    protected function setupMasterParms()
    {
        ...
        if (($master = Get(Config("TABLE_SHOW_MASTER"), Get(Config("TABLE_MASTER")))) !== null) {
            ...
            if ($masterTblVar == "cis_refs") {
                ...
                if (($parm = Get("fk_Nidentifier", Get("cisrefs_id"))) !== null) {
                    $masterTbl->Nidentifier->setQueryStringValue($parm);
                    $this->cisrefs_id->setQueryStringValue($masterTbl->Nidentifier->QueryStringValue);
                    $this->cisrefs_id->setSessionValue($this->cisrefs_id->QueryStringValue);
                } else {
                    $validMaster = false;
                }
            }
            ...
            if ($masterTblVar == "cis_refs") {
                $validMaster = true;
                $masterTbl = Container("cis_refs");
                if (($parm = Post("fk_Nidentifier", Post("cisrefs_id"))) !== null) {
                    $masterTbl->Nidentifier->setFormValue($parm);
                    $this->cisrefs_id->setFormValue($masterTbl->Nidentifier->FormValue);
                    $this->cisrefs_id->setSessionValue($this->cisrefs_id->FormValue);
                } else {
                    $validMaster = false;
                }
            }
        ...
        if ($validMaster) {
            ...
            if ($masterTblVar != "cis_refs") {
                if ($this->cisrefs_id->CurrentValue == "") {
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("SubcatCisLinksList"), "", $this->TableVar, true);
