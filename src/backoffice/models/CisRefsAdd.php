 ...
 * Page class
 */
class CisRefsAdd extends CisRefs
{
    use MessagesTrait;
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$loaded) { // Record not loaded
                    ...
                    $this->terminate("CisRefsList"); // No matching record, return to list
                    return;
                }
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($this->OldRecordset)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "CisRefsList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "CisRefsView") {
    ...
    protected function validateForm()
    {
        ...
        if (in_array("subcat_cis_links", $detailTblVar) && $detailPage->DetailAdd) {
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            ...
            $detailPage = Container("SubcatCisLinksGrid");
            if (in_array("subcat_cis_links", $detailTblVar) && $detailPage->DetailAdd) {
                $detailPage->cisrefs_id->setSessionValue($this->Nidentifier->CurrentValue); // Set master key
                $addRow = $detailPage->gridInsert();
                if (!$addRow) {
                $detailPage->cisrefs_id->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
