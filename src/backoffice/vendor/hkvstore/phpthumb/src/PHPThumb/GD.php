...
class GD extends PHPThumb
{
    ...
    public function resize($maxWidth = 0, $maxHeight = 0)
    {
        ...
        if ($this->options['keepAspectRatio'] == false && $maxWidth > 0 && $maxHeight > 0) { // do not keep aspect ratio
            $this->newDimensions = array('newWidth' => $maxWidth, 'newHeight' => $maxHeight);
        } else {
    ...
    /**
     * Resizes image (extended)
     *
     * @param string $data Get image as string if $fn not specified
     * @param int $width
     * @param int $height
     * @param string $fn The file name to save
     * @return void
     */
    public function resizeEx(&$data, &$width, &$height, $fn = "")
    {
        if ($this->plugins) { // Execute plugins
